...
"""Ansible module for manage nemo evaluators."""
...
short_description: Manage NeMo evaluators
description:
    - Manage NeMo evaluators using the NVIDIA REST API.
...
EXAMPLES = r"""
- name: Create a evaluator
  stevefulme1.nvidia_nemo.nemo_evaluator:
    name: "example-name"
    state: present

- name: Delete a evaluator
  stevefulme1.nvidia_nemo.nemo_evaluator:
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/evaluators/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/evaluators", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/evaluators/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/evaluators/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/evaluators/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/evaluators/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/evaluators/{existing['id']}",
...
def main():
    ...
    if params.get("evaluator_id"):
        existing = get_resource(client, params["evaluator_id"])
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a evaluator.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, evaluator=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, evaluator=to_dict(existing))
