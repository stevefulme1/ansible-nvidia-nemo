...
"""Ansible module for manage nemo fine-tuning jobs."""
...
DOCUMENTATION = r"""
---
module: nemo_fine_tune
short_description: Manage NeMo fine-tuning jobs
description:
    - Manage NeMo fine-tuning jobs using the NVIDIA REST API.
...
options:
    ...
    base_model_id:
        description:
            - The base model id for the fine tune.
        type: str
    dataset_id:
        description:
            - The dataset id for the fine tune.
        type: str
    state:
        description:
...
- name: Create a fine tune
  stevefulme1.nvidia_nemo.nemo_fine_tune:
    name: "example-name"
    base_model_id: "example-base_model_id"
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/fine_tunes/{resource_id}")
...
def create_resource(module, client):
    ...
    create_data = {k: v for k, v in {
        "name": params.get("name"), "base_model_id": params.get("base_model_id"), "dataset_id": params.get("dataset_id")
    }.items() if v is not None}
    resource = client.post("/v1/fine_tunes", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/fine_tunes/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/fine_tunes/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/fine_tunes/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/fine_tunes/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/fine_tunes/{existing['id']}",
...
def needs_update(params, existing):
    ...
    updatable = ['name', 'base_model_id', 'dataset_id']
...
def main():
    ...
    if existing is None:
        for req in ['name', 'base_model_id', 'dataset_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a fine tune.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, fine_tune=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, fine_tune=to_dict(existing))
