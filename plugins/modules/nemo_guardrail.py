...
"""Ansible module for manage nemo guardrails."""
...
DOCUMENTATION = r"""
---
module: nemo_guardrail
short_description: Manage NeMo guardrails
description:
    - Manage NeMo guardrails using the NVIDIA REST API.
...
options:
    guardrail_id:
        description:
            - The guardrail id for the guardrail.
        type: str
    name:
        description:
...
EXAMPLES = r"""
- name: Create a guardrail
  stevefulme1.nvidia_nemo.nemo_guardrail:
    name: "example-name"
    state: present

- name: Delete a guardrail
  stevefulme1.nvidia_nemo.nemo_guardrail:
    guardrail_id: "example-id"
    state: absent
"""
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/guardrails/{resource_id}")
...
def find_resource(client, name):
    ...
    if not name:
        return None
    try:
        result = client.get("/v1/guardrails")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/guardrails", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/guardrails/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/guardrails/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/guardrails/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/guardrails/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/guardrails/{existing['id']}",
...
def main():
    ...
    existing = None
    if params.get("guardrail_id"):
        existing = get_resource(client, params["guardrail_id"])
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a guardrail.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, guardrail=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, guardrail=to_dict(existing))
