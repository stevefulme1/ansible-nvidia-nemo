...
"""Ansible module for manage nemo retrievers."""
...
DOCUMENTATION = r"""
---
module: nemo_retriever
short_description: Manage NeMo retrievers
description:
    - Manage NeMo retrievers using the NVIDIA REST API.
...
EXAMPLES = r"""
- name: Create a retriever
  stevefulme1.nvidia_nemo.nemo_retriever:
    name: "example-name"
    state: present

- name: Delete a retriever
  stevefulme1.nvidia_nemo.nemo_retriever:
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/retrievers/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/retrievers", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/retrievers/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/retrievers/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/retrievers/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/retrievers/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/retrievers/{existing['id']}",
...
def main():
    ...
    if params.get("retriever_id"):
        existing = get_resource(client, params["retriever_id"])
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a retriever.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, retriever=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, retriever=to_dict(existing))
